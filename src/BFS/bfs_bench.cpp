#if defined(BFS_STRICT)
#include "bfs_strict.h"
#else
#include "bfs.h"
#endif
...
int main(int argc, char *argv[]) {
  ...
  BFS<decltype(graph), chunkbag> solver(graph);
#else
  BFS solver(graph);
#endif
  for (int repetition = -1; repetition < measured_rounds; repetition++) {
    ...
    solver.run_prepared();
    timer.stop();
    printf("BFS_BENCH_TIME\t%s\t%.9f\n",
           repetition < 0 ? "warmup" : std::to_string(repetition).c_str(),
           timer.total_time());
    fflush(stdout);
    if (repetition < 0) {
      write_distances(distance_path, graph.n, source, solver.distances());
    }
  }
  return EXIT_SUCCESS;
}
