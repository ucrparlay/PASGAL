...
#ifdef STAT
  for (auto qsize : {10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000,
                     50000, 100000, 200000, 500000, 1000000}) {
    // for (auto qsize : {2000}) {
#else
  // for (auto qsize : {10, 50, 100, 500, 1000, 5000, 10000, 50000, 100000}) {
  for (auto qsize : {500000, 1000000}) {
#endif
    solver.LOCAL_QUEUE_SIZE = qsize;
    all_time = 0;
    if (source == UINT_MAX) {
      run(solver, G, verify);
      all_time /= NUM_SRC;
